...
public class SessionControler extends AbstractVerticle {
    ...
    public void start() {
        ...
        router.get("/session/").handler(this::getSessions);
    ...
    private void postNewSession(RoutingContext rc){
        try {
            ...
            if(googleID != null){
                Logger.getLogger(this.getClass().getName()).log(Level.INFO, "Player login : {0}", googleID);
                sessionModel.putSession(sessionID, googleID);
                rc.response().putHeader("content-type", "application/json")
                        .end(new JsonObject().put("result", "OK").encodePrettily());
            }else{
                Logger.getLogger(this.getClass().getName()).log(Level.WARNING, "Invalid idtoken");
            }
    ...
    private void getSessions(RoutingContext rc){
        sessionModel.getSessions(sessions -> {
            rc.response()
